...
void    settings_options_update(void)
{
	...
	update_textBox(&g_settings_options.testEntry, adjust, CAM07_DEFAULT_FONT, CAM07_SPACING, (intptr_t)NULL);
	...
	BeginTextureMode(g_engine->allCameras->camera07.textForCam);
		...
		BeginMode2D(g_engine->allCameras->camera07.camera2D);
			...
			DrawTextEx(CAM07_DEFAULT_FONT, "Settings Options", (Vector2){rect.width / 2 - MeasureText("Settings Options", 30 + CAM07_FONT_ADJUST) / 2, 20}, 30, CAM07_SPACING, CAM07_MAIN_TEXT_COLOR);
			...
			draw_textBox(g_settings_options.testEntry, CAM07_DEFAULT_FONT, VERRYDARKGRAY, CAM07_SPACING, (intptr_t)NULL);
