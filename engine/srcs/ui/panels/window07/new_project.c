...
void    new_project_init(void)
{
	...
	g_files_new.projectNameEntry = create_textBox(rec, 64);
}
...
void    new_project_update(void)
{
	...
	BeginTextureMode(g_engine->allCameras->camera07.textForCam);
		...
		BeginMode2D(g_engine->allCameras->camera07.camera2D);
			...
			DrawTextEx(CAM07_DEFAULT_FONT, "New Project", (Vector2){rect.width / 2 - MeasureText("New Project", 30 + CAM07_FONT_ADJUST) / 2, 20}, 30, CAM07_SPACING, CAM07_MAIN_TEXT_COLOR);
