...
FilesExport g_files_export;

void    export_project_init(void)
{
    Rectangle rec = { 20, 65, 300, 22};
	g_files_export.testEntry = create_textBox(rec, 64);
}
...
void    export_project_update(Engine *engine)
{
	...
	update_textBox(&g_files_export.testEntry, adjust, CAM07_DEFAULT_FONT, CAM07_SPACING, (intptr_t)NULL);
	...
	BeginTextureMode(engine->allCameras->camera07.textForCam);
		...
		BeginMode2D(engine->allCameras->camera07.camera2D);
			...
			DrawTextEx(CAM07_DEFAULT_FONT, "Export Project", (Vector2){rect.width / 2 - MeasureText("Export Project", 30 + CAM07_FONT_ADJUST) / 2, 20}, 30, CAM07_SPACING, CAM07_MAIN_TEXT_COLOR);
			...
			draw_textBox(g_files_export.testEntry, CAM07_DEFAULT_FONT, VERRYDARKGRAY, CAM07_SPACING, (intptr_t)NULL);
...
{
    g_files_export.testEntry.text[0] = '\0';
	g_files_export.testEntry.textSize = 0;
}

void    export_project_destroy(void)
{
    destroy_textBox(&g_files_export.testEntry);
}
