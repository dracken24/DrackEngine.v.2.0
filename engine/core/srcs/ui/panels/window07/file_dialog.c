...
void draw_file_dialog(FileDialog* dialog, Vector2 camPos)
{
	...
	Vector2 textSize = MeasureTextEx(font, dialog->currentPath, 14, 1);
	textSize.x += MeasureTextEx(font, "Path: ", 14, 1).x;
	
	if (textSize.x > dialog->size.x - 20)
	{
		const float maxWidth = dialog->size.x - 80 - MeasureTextEx(font, "Path: ", 14, 1).x;
		char displayText[1024];
		char tempText[1024];
		strncpy(tempText, dialog->currentPath, sizeof(tempText) - 1);
		tempText[sizeof(tempText) - 1] = '\0';
		
		int startPos = 0;
		int lineCount = 0;
		
		while (startPos < strlen(tempText) && lineCount < 2)
		{
			int charCount = 0;
			float currentWidth = 0;
			
			// Calcul how many characters can fit on a line
			while (currentWidth < maxWidth && (startPos + charCount) < strlen(tempText))
			{
				char nextChar[2] = {tempText[startPos + charCount], '\0'};
				Vector2 charSize = MeasureTextEx(font, nextChar, 14, 1);
				if (currentWidth + charSize.x > maxWidth) break;
				currentWidth += charSize.x;
				charCount++;
			}
			
			// Copy the portion of the text that can fit on a line
			strncpy(displayText, tempText + startPos, charCount);
			displayText[charCount] = '\0';
			
			// Display the line
			DrawTextEx(font, displayText, 
				(Vector2){dialog->position.x + 10 + 
					(lineCount == 0 ? MeasureTextEx(font, "Path: ", 14, 1).x : 0),
					dialog->position.y + dialog->size.y - 60 + (lineCount * 15)},
				14, 1, DARKGRAY);
